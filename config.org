...
* Introduction
  ...
  all the configuration for GNU Emacs. But we rather want to use this very file
  you are reading right now, since it aims at describing the configuration to
  you, and therefore a better understanding of the configuration by
  you. Therefore we have to parse this config file.
  ...
  Resulting of this requirement, our emacs init file needs the following code:
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    ;; system. If you have multiple files, add them accordingly. I suggest, you use
    ;; a general config file and one for modifications for your specific system.
  ...
  visit this file in my [[https://github.com/chrugi/Emacs/][Github repository]]. So, let's go!
...
* Common Lisp Integration
  ...
  In order to being able to use common lisp we load the =cl= package.
...
** GNU Emacs Package Manager
  ...
  additional packages that increase the functionality of our GNU Emacs setup.
...
** try
   ...
   install a package called "try", which gives us the ability to temporarely run
   the package we want to try out, without actually installing it in the system.
   ...
   You can then simply try a package with "M-x try" followed by the name of the
   package.

   the =:ensure t= part you will meet in almost every package we configure
   through use-package. It simply means, that use-package will check on launch
   whether the package executables are available, and if not will install them.
